...
public class TCMOntologyJobWorker {
    ...
    /**
     * 带「realize 命中集」「患者六经」「患者八纲」的打分排序。
     *
     * <p>排序规则（依次）：
     * <ol>
     *   <li><b>方证相应</b>（依《伤寒论》六经八纲辨证）：
     *     <ul>
     *       <li>0a. 双方均 realize（完全命中）时，先按「病位覆盖」裁决
     *           （{@link #compareBingweiCoverage}：合病须合治，方证病位须覆盖患者病位），
     *           再按父子层级（子方优先，依「方随证转」）。</li>
     *       <li>0b. 兜底（非 realize）路径，jaccard 接近（&lt;0.15）时同样先比病位覆盖、再比父子。</li>
     *     </ul>
     *   </li>
     *   <li>1. 广谱高分方证惩罚；2. jaccard 降序；3. hits 降序；4. required 降序；
     *       5. clinicalPriority 升序；5.5 方证六经与患者六经相符度（末位裁决）；6. 字典序兜底。</li>
     * </ol>
     *
    ...
    private List<ScoredFangzheng> rankCandidatesWithScores(
        ...
        return candidates.stream()
                ...
                .sorted((a, b) -> {
                    ...
                    if (cmp != 0) return cmp;
                    // 5.5 方证六经与患者六经相符度（末位裁决）：
                    //     方证相应须六经相应（胡希恕「先辨六经，继辨方证」）。
                    //     对称差小者优先；任一方无六经归属时不介入（无意见）。
