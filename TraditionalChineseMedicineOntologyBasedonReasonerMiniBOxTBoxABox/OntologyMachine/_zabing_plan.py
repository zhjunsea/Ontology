...
for f in files:
    ...
    for m in re.finditer(r'rdf:about="#([^"]+)_instance"', txt):
        instances.add(m.group(1))
classes = set(n for n in defined if not n.endswith("_instance"))
...
print("=== 逐用例：测试无实例名 / 本体filler悬空 / 差异 ===")
for m in re.finditer(r'void (t_\w+)\(\)\s*\{\s*assertFangzheng\((.*?)\);\s*\}', src, re.S):
    ...
    missing_inst = [n for n in syms + pulses if n not in instances]
    dang = [v for k, v in fills if v not in defined]
    only_test = [s for s in syms if s not in fill_syms]
    only_onto = [s for s in fill_syms if s not in syms]
    if not (missing_inst or dang or only_test or only_onto):
        continue
    print(f"\n### {tname}  fz={fz}")
    print(f"    测试症状 : {';'.join(syms)}")
    print(f"    本体filler: {';'.join(fill_syms)}")
    if fill_mais: print(f"    本体脉象 : {';'.join(fill_mais)}  | 测试脉象: {';'.join(pulses)}")
    if missing_inst: print(f"    [测试无实例] {missing_inst}")
    for n in missing_inst:
        tag = "有类无实例" if n in classes else "无类"
        tag += " / filler引用" if n in fill_syms + fill_mais else " / 仅测试多传"
        print(f"        {n}: {tag}")
    if dang: print(f"    [本体filler悬空] {dang}")
