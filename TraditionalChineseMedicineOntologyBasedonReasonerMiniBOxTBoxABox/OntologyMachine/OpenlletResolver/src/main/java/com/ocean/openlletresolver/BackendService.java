...
public class BackendService implements AutoCloseable {
    ...
    */
    ...
    private void collectSubclasses(OWLClass cls, Set<OWLClass> acc, Set<OWLClass> visited) {
        ...
        for (OWLSubClassOfAxiom axiom : ont.getAxioms(AxiomType.SUBCLASS_OF)) {
     ...
     *
     * @param iriOrFragment IRI 或 fragment
     * @param baseNamespace 基础命名空间（例如 "http://example.org/ontology#"）
     * @return 中文标签；若未找到，则返回原输入（fragment 或完整 IRI）
     */
    public String resolveLabel(String iriOrFragment, String baseNamespace) {
        if (iriOrFragment == null) return null;
        String fullIri = toFullIri(iriOrFragment, baseNamespace);
        return resolveLabel(fullIri);
    }

    /**
     * 批量解析中文标签（支持 fragment 自动补全）。
     *
     * @param iris          IRI 或 fragment 列表
     * @param baseNamespace 基础命名空间
     * @return 中文标签列表（顺序与输入一致）
     */
    public List<String> resolveLabels(List<String> iris, String baseNamespace) {
        if (iris == null) return List.of();
        return iris.stream()
                .map(iri -> resolveLabel(iri, baseNamespace))
                .collect(Collectors.toList());
    }

    /**
     * 将 fragment 转换为完整 IRI；若已是完整 IRI 则原样返回。
     *
     * @param iriOrFragment IRI 或 fragment
     * @param baseNamespace 基础命名空间
     * @return 完整 IRI
     */
    private String toFullIri(String iriOrFragment, String baseNamespace) {
        if (iriOrFragment.startsWith("http://") || iriOrFragment.startsWith("https://")) {
