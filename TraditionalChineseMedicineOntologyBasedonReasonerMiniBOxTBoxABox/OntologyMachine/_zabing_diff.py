...
for f in files:
    txt = open(f, encoding="utf-8").read()
    marks = [(m.start(), m.group(1)) for m in re.finditer(r'<owl:Class rdf:about="#([^"]+)"', txt)]
    for i, (pos, name) in enumerate(marks):
        ...
        fills = re.findall(r'<owl:onProperty rdf:resource="#(you_\w+)"/><owl:someValuesFrom rdf:resource="#([^"]+)"', eq)
        if fills:
...
for m in re.finditer(r'void (t_\w+)\(\)\s*\{\s*assertFangzheng\((.*?)\);\s*\}', src, re.S):
    ...
    if not fills:
        print(f"### {tname}  fz={fz}  [本体无等价类filler]")
        continue
    print(f"### {tname}  fz={fz}")
    print(f"    测试症状 : {';'.join(syms)}")
    print(f"    本体filler: {';'.join(fill_syms)}")
    if fill_mais:
        print(f"    本体脉象 : {';'.join(fill_mais)}")
    print(f"    测试脉象 : {';'.join(pulses)}")
    # 差异
